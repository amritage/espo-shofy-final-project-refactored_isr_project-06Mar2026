...
export default function ProductDetailsClient({ productId, initialProduct = null }) {
  const cleanProductId = productId ? String(productId).replace(/#$/, '') : productId;

  const { data, isLoading, isError, error } = useGetSingleNewProductQuery(cleanProductId, {
    skip: !cleanProductId || Boolean(initialProduct),
  });

  if (initialProduct) {
    const product = mapBackendProductToFrontend(initialProduct?.data ?? initialProduct);
    return product ? <ProductDetailsArea product={product} /> : <ErrorMsg msg="Product data could not be processed" />;
  }

  if (isLoading) return <ProductDetailsLoader />;
  if (isError) return <ErrorMsg msg={`Error loading product: ${error?.status || 'Unknown error'}`} />;
  if (!data?.data) return <ErrorMsg msg="No product found!" />;

  const product = mapBackendProductToFrontend(data.data);
  if (!product) return <ErrorMsg msg="Product data could not be processed" />;
