...
export default function PopularProducts({ initialProducts = null }) {
  const hasInitialProducts = Array.isArray(initialProducts) && initialProducts.length > 0;
  const { data: sharedData, isError, isLoading, error } = useGetAllProductsForFilteringQuery(undefined, {
    skip: hasInitialProducts,
  });
  const swiperRef = useRef(null);
  const sourceData = hasInitialProducts
    ? { data: initialProducts, total: initialProducts.length, success: true }
    : sharedData;
  ...
  const data = React.useMemo(() => {
    ...
    const products = sourceData.data;
  ...
  if (!hasInitialProducts && isLoading) carousel = <HomeTwoPopularPrdLoader loading />;
  if (!hasInitialProducts && !isLoading && isError) {
    carousel = (
